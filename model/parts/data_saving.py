...
def extract_common_data(params, state):
    ...
    common_data = {key: state[key] for key in ["seed", "first_seal_rage_quit_support", "second_seal_rage_quit_support"]}
    ...
    for i, reaction_time in enumerate(ReactionTime):
        common_data[reaction_time.name] = np.sum(actors.reaction_time == i)

    for i, actor_type in enumerate(ActorType):
        common_data[actor_type.name] = np.sum(actors.actor_type == i)
...
def save_common_dataframe(params, substep, state_history, prev_state, policy_input):
    ...
    combined_df = pd.concat([prev_state["common_dataframe"], common_data_df], ignore_index=True)
    ...
    if timestep == prev_state["n_timesteps"]:
        ...
        if proposal_data_df is not None:
            path = prev_state["outpath"].joinpath("proposals_data.parquet")
            proposal_data_df.to_parquet(path, index=False)

    return ("common_dataframe", combined_df)
