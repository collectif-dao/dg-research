...
def digest_and_olden(params, substep, state_history, prev_state):
    ...
    return {"agent_delta_food": delta_food, "agent_delta_age": delta_age}
...
def reproduce_agents(params, substep, state_history, prev_state):
    ...
    for agent_type in set(agent["type"] for agent in agents.values()):
        ...
        for agent_label, agent_properties in specific_agents.items():
            ...
            if (
                agent_properties["food"] < food_threshold
                or agent_label in already_reproduced
            ):
                continue
            kind_neighbors = nearby_agents(location, specific_agents)
            available_partners = [
                label
                for label, agent in kind_neighbors.items()
                if agent["food"] >= food_threshold and label not in already_reproduced
            ]
...
def feed_prey(params, substep, state_history, prev_state):
    ...
    hungry_preys = {
        label: properties
        for label, properties in preys.items()
    ...
    return {"agent_delta_food": agent_delta_food, "site_delta_food": site_delta_food}
...
def hunt_prey(params, substep, state_history, prev_state):
    ...
    preys = {k: v for k, v in agents.items() if v["type"] == "prey"}
    predators = {k: v for k, v in agents.items() if v["type"] == "predator"}
    hungry_predators = {
        k: v for k, v in predators.items() if v["food"] < hungry_threshold
    }
    agent_delta_food = {}
...
def agent_remove(params, substep, state_history, prev_state, policy_input):
    ...
    surviving_agents = {
        k: v for k, v in prev_state["agents"].items() if k not in agents_to_remove
    }
